...
def test_checks_clean_fixture(tmp_path):
    for name in ('AGENTS.md','CONTRIBUTING.md','CHANGELOG.md','docs/INDEX.md','docs/STATUS.md'):
        write(tmp_path/name, '# file')
    write(tmp_path/'docs'/'a.md', '---\nid: TASK-1\ntype: task\nstatus: draft\ncreated: 2026-09-02\nupdated: 2026-09-02\n---\n[ok](../AGENTS.md)')
...
def test_checks_find_broken_links_in_plain_markdown_and_ignores_git_and_temp(tmp_path):
    ...
    result = run_checks(tmp_path)
    assert sum(i['code'] == 'broken_link' for i in result.issues) == 5
